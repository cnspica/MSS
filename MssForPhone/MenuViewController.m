...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        [self initlabel];
        [self.tabBarItem setImage:[UIImage imageNamed:@"menu.png"]];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    //    NSLog(@"%@",api_language);
    apistring=[NSString stringWithFormat:@"%@?lang=%@",HTTP_menu,api_language];
...
{
    ptname.text=[NSString stringWithFormat:@"%@",[[[datadic objectForKey:@"data"]objectAtIndex:ptname.tag-1] objectForKey:@"menu"]];
}
